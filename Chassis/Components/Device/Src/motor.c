...
{
	/* Judge the StdId */
	uint8_t Motor_ID = *StdId-0x241;
  if(RMD_Motor->Data.Initlized == 0){
	LowPassFilter1p_Init(&RMD_Motor_Filterlp[Motor_ID],0.1,0.001f);
	RMD_Motor->Data.Initlized = 1;
	}
  /* Update the receive order */
	...
	RMD_Motor->Data.Velocity = ((int16_t)(rxBuf[5]<<8) | (int16_t)(rxBuf[4]));
	if(abs(RMD_Motor->Data.Velocity)<40) RMD_Motor->Data.Filter_Velocity = LowPassFilter1p_Update(&RMD_Motor_Filterlp[Motor_ID],RMD_Motor->Data.Velocity);
	else  RMD_Motor->Data.Filter_Velocity =   RMD_Motor->Data.Velocity;
	RMD_Motor->Data.Rad_Velocity =  RMD_Motor->Data.Filter_Velocity * Angle_to_rad;
