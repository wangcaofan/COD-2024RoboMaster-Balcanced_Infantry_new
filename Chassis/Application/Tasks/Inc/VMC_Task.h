...
typedef struct 
{
  float L1,L2,L3,L4,L5,LBD,L0,Predict_L0,Target_L0,Last_Target_L0;
	float *Phi1,*Phi4;
	float  Phi2,Phi3,Phi0;
	float  Predict_Phi1,Predict_Phi4,Predict_Phi2,Predict_Phi0;
	float *Phi1_dot,*Phi4_dot,Phi0_dot,Filter_Phi0_dot,Last_Phi0;
	float X_B,Y_B,X_D,Y_D,X_C,Y_C,Y_C_dot,X_C_dot;
	float F,T1,T2,Tp;
	float Theta;
	float Predict_Theta;
	VMC_USAGE_e VMC_USAGE;
	LowPassFilter1p_Info_TypeDef Phi0_dot_LowPassFilter;
